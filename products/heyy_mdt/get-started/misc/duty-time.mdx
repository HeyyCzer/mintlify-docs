...
```lua server.lua
TriggerEvent("heyy_mdt:setOnDuty", source, true) -- Inicia a contagem de horas
```
...
```lua server.lua
TriggerEvent("heyy_mdt:setOnDuty", source, false) -- Finaliza a contagem de horas
```
...
AddEventHandler("Connect", function(user_id, source) -- Evento disparado quando o jogador entra no servidor
	...
	if vRP.HasService(user_id, "Police") then -- verifica se o jogador está em serviço como Policial
		TriggerEvent("heyy_mdt:setOnDuty", source, true) -- Inicia a contagem de horas novamente
	end
end)
```
